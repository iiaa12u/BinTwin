...
OVERDUE_HOURS = 48
RESET_FILL_AFTER_COLLECTION = 5

# Static fixed collection windows
STATIC_WINDOWS = [
    ("15:00", "17:00"),
    ("20:00", "22:00"),
]

# Truck start locations
...
def build_distance_matrix(nodes):
    return [
        [
            int(round(haversine_km(a["lat"], a["lng"], b["lat"], b["lng"]) * 1000))
            for b in nodes
        ]
        for a in nodes
    ]
...
def solve_single_trip(payload, candidates, depot_lat, depot_lng):
    ...
    routing.SetArcCostEvaluatorOfAllVehicles(
        time_cb_idx if goal == "time" else distance_cb_idx
    )
...
def build_route_timeline(
    visited,
    truck_capacity_kg=TRUCK_CAPACITY_KG_DEFAULT,
    enforce_max_stops=True,
    shift_window_label="Flexible Dynamic Window",
):
    """
    Build a route timeline with truck capacity handling.

    If the next bin would exceed truck capacity:
    truck goes to landfill -> unloads -> continues remaining bins.
    """
    ...
    depot = campus_depot_map.get(first_side, EAST_DEPOT)
    ...
    def make_landfill_node(side):
        nonlocal dump_count
        dump_count += 1

        return {
            "node_id": f"LANDFILL-{dump_count}",
            "node_type": "LANDFILL",
            "lat": LANDFILL["lat"],
            "lng": LANDFILL["lng"],
            "side": side,
    ...
    for b in visited:
        ...
        next_demand = int(b.get("demand_kg", demand_kg(b["current_fill"])))

        capacity_full = onboard_kg > 0 and onboard_kg + next_demand > truck_capacity_kg
        max_stops_reached = enforce_max_stops and trip_stops >= MAX_STOPS_PER_TRIP

        if capacity_full or max_stops_reached:
            landfill_node = make_landfill_node(cur_node["side"])
        ...
        serviced_rows.append(
            {
                ...
                "shiftWindow": shift_window_label,
            }
        )
...
def build_static_capacity_timeline(
    visited,
    truck_capacity_kg=TRUCK_CAPACITY_KG_DEFAULT,
):
    """
    Static baseline:
    - Fixed scheduled collection windows
    - Fixed threshold selection
    - Fixed order
    - Truck capacity constraint is applied
    """
    ...
    ordered_bins = sorted(
        [dict(b) for b in visited],
        key=lambda b: (b.get("side", ""), b.get("id", "")),
    )

    # Split static route across two fixed windows.
    midpoint = (len(ordered_bins) + 1) // 2
    grouped_bins = [ordered_bins[:midpoint], ordered_bins[midpoint:]]
    ...
    for window_index, window_bins in enumerate(grouped_bins):
        if not window_bins:
            continue

        window_start, window_end = STATIC_WINDOWS[
            min(window_index, len(STATIC_WINDOWS) - 1)
        ]

        shift_label = f"{window_start}-{window_end}"

        rows, serviced_rows, distance_km, duration_min = build_route_timeline(
            window_bins,
            truck_capacity_kg=truck_capacity_kg,
            enforce_max_stops=False,
            shift_window_label=shift_label,
        )

        for item in serviced_rows:
            item["eta"] = eta_from_minutes(window_start, int(item["arrive_min"]))
            item["shiftWindow"] = shift_label

        all_rows.extend(rows)
        all_serviced_rows.extend(serviced_rows)
        total_km += distance_km
        total_time_min += duration_min

    return all_rows, all_serviced_rows, round(total_km, 2), round(total_time_min)
...
def empty_response(snapshot, threshold, mode):
    avg_fill = (
        round(sum(b["current_fill"] for b in snapshot) / len(snapshot), 2)
        if snapshot
        else 0
    )
    ...
    return {
        "kpis": {
            ...
            "overThreshold": len(
                [b for b in snapshot if b["current_fill"] >= threshold]
            ),
        },
        "summary": {
            "totalBins": 0,
...
def solve_route(payload: dict):
    ...
    if not must_bins and not optional_bins:
        return empty_response(snapshot, threshold, mode)

    all_candidates = must_bins + optional_bins

    if mode == "STATIC":
        visited = sorted(
            [dict(b) for b in all_candidates],
            key=lambda b: (b.get("side", ""), b.get("id", "")),
        )

        timeline_rows, serviced_rows, total_distance_km, total_time_min = (
            build_static_capacity_timeline(
                visited,
                truck_capacity_kg=truck_capacity_kg,
            )
        )
        ...
        first_side = all_candidates[0]["side"].lower()
    ...
    for idx, item in enumerate(serviced_rows, start=1):
        ...
        if mode == "STATIC":
            fill_in = item.get("shiftWindow", "Fixed Shift")
            eta = item.get("eta", eta_from_minutes(STATIC_WINDOWS[0][0], int(item["arrive_min"])))
        else:
            fill_in = (
                "Now"
                if b["current_fill"] >= threshold or b["overdue"]
                else f"Within {forecast_hours}h"
            )
    ...
    total_collected_kg = sum(b["demand_kg"] for b in visited)
    truck_capacity_pct = (
        0
        if truck_capacity_kg == 0
        else min(100, round((total_collected_kg / truck_capacity_kg) * 100))
    )
    ...
    return {
        ...
        "driverRoute": {
            ...
            "truckCapacityPct": truck_capacity_pct,
            "stops": driver_stops,
        },
    }